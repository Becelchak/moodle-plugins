...
class mod_webgl_renderer extends plugin_renderer_base {
    /**
     * Returns the header for the lesson module
     *
     * @param lesson $webgl a webgl object.
     ...
     * @return string
     */
    public function header($webgl, $cm, $currenttab = '', $extraeditbuttons = false, $lessonpageid = null, $extrapagetitle = null) {
        global $CFG, $USER;

        $activityname = format_string($webgl->name, true, $webgl->course);
        ...
        $canmanage = has_capability('mod/webgl:manage', $context);
        ...
        if ($activityheader->is_title_allowed()) {
            $title = $canmanage && $setactive ?
                $this->output->heading_with_help($activityname, 'overview', 'webgl') :
                $activityname;
        ...
        foreach ($webgl->messages as $message) {
     ...
     *
     * @param object $webgl The webgl that the user is taking.
     * @return void
     **/
    ...
    /**
     * Returns HTML to display a progress bar of progression through a webgl
     *
     * @param lesson $webgl
